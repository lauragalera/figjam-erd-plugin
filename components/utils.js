...
function estimateTextWidth(text, offsetNull, charWidth = 10, padding = 40) {
    return text.length * charWidth + padding + (offsetNull ? 50 : 0);
}
export function calculateMaxWidth(tableHeader, tableData) {
    var _a, _b;
    let maxWidth = 0;
    ...
    for (const field of (_a = tableData.fields) !== null && _a !== void 0 ? _a : []) {
        const fieldWidth = estimateTextWidth(`${field.name} ${field.type}`, (_b = field.not_null) !== null && _b !== void 0 ? _b : false);
...
export function createSectionWithNodes(allTables, allConnectors, sectionName) {
    const MARGIN = 100;
    ...
    section.name = sectionName !== null && sectionName !== void 0 ? sectionName : 'Untitled Section';
...
export async function createTextNode(characters, bold = false, fontSize = theme.fontSize) {
...
export async function placeWithoutOverlap(tables) {
...
export async function createLegend(x = 0, y = 0) {
